...
Before editing, use `superpowers:using-git-branches` in `implementation-start` mode. Work in the current checkout. If the current branch is `main` or `master`, the branch skill creates a task branch before implementation starts.
...
- **superpowers:using-git-branches** - Protects the branch before implementation.
