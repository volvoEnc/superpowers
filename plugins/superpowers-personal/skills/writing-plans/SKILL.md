...
The main agent is the orchestrator, not the plan author. It prepares durable inputs, dispatches fresh helpers, captures their outputs, closes them, and stores artifacts. When helper/subagent support is available, do not write or review the implementation plan inline.
...
The main agent reads only the approved spec, decision pack/handoff, and minimal repository metadata needed to dispatch helpers.

### 2. Dispatch context-scout helper

Give the helper only the approved spec, decision pack/handoff, and repository paths to inspect.
...
Capture the result as `context-pack.md`, then close the helper immediately.

### 3. Dispatch plan-author helper

Give the helper only:
...
Capture the result, write the plan files, then close the helper immediately.

### 4. Review the plan with fresh helpers
...
After each reviewer returns, capture its receipt and close that reviewer immediately.
...
Keep it concise. It is sterile context for helpers, not a chat transcript.
...
1. Subagent-Driven - fresh helper per task with review between tasks
...
If helper/subagent support is unavailable, the main agent may write and review the plan inline. It must still use the approved spec and context pack as the only source of truth and must not carry brainstorming history into execution.
