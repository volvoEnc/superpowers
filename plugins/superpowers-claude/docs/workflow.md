# Workflow разработки Superpowers (spec → plan → review → build → PR)

Документ описывает, как устроен конвейер разработки в этом форке: какие скилы и
инструменты в нём участвуют, **когда** каждый вызывается и **зачем**. Соседние
доктрины: [`review-integration-doctrine.md`](review-integration-doctrine.md),
[`liveness-doctrine.md`](liveness-doctrine.md).

## 1. Зачем это нужно

Когда партнёр говорит «давай сделаем X», агент **не бросается писать код**. Вместо
этого он проходит дисциплинированный конвейер: выясняет, что нужно на самом деле →
фиксирует это в **спеке** (единственный человеческий гейт одобрения) → строит из неё
план → проверяет план до кода → исполняет задачу за задачей **свежими субагентами** →
открывает PR.

Две идеи держат всю систему:
