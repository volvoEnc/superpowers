...
- **Wall-clock floor (liveness):** the orchestrator evaluates the floor against `state.json` `inflight` entries **between tasks**, at **`ScheduleWakeup` wake points**, and **on resume after a compact** — the floor is the only liveness signal that survives compaction. Make the floor **executable** between tasks: run `../../scripts/liveness-floor.sh <state.json>` over the open `inflight[]`. It is detect-only — it prints `STALE` for any unit past `G × deadline_s`, and the orchestrator decides the response; for each `STALE` line, enter the response path in `../../docs/liveness-doctrine.md` (§7). Field shapes are single-sourced in `superpowers:phase-handoff`; reference, do not redefine the floor mechanics here. See `../../docs/liveness-doctrine.md`.
