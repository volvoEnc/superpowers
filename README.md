...
Это [obra/superpowers](https://github.com/obra/superpowers) с несколькими
доработками — в основном под **долгие автономные прогоны** и под работу в
**Claude Code**.

| Что нового | Какую проблему решает |
|---|---|
| **Доктрина живости фоновых помощников** | Если фоновый помощник завис или «умер» на долгой задаче, главный агент может ждать его бесконечно. Форк задаёт **поведенческий триггер по времени**: скилы предписывают агенту замечать просрочку по «настенным часам», спасать уже сделанное и при необходимости перезапускать или звать человека. Это **доктрина и дисциплина** поверх встроенных средств Claude Code, а не отдельный сторожевой процесс; несущая страховка — пассивный пол по wall-clock, переживающий компакцию. Полноценный рантайм-мониторинг — в планах. |
| **Фоновый веер — свободный ввод** | При 2+ независимых субагентах оркестратор запускает их в фон и **возвращает ход тебе**: поле ввода не блокируется, пока веер работает, а результаты собираются по мере готовности (см. `dispatch-yield-doctrine.md`). |
| **Предсказуемый план и его проверка** | План собирается по понятным шагам, а проверяется сразу с нескольких сторон, перепроверяя каждое замечание. Результат меньше зависит от везения. |
| **Обычные ветки вместо отдельных рабочих копий** | В оригинале под задачу заводится отдельная копия проекта (worktree) — для одиночной работы это лишняя возня. Здесь — простая ветка. |
| **Ближе к встроенным средствам Claude Code** | Фоновые задачи, авто-проверка кода, уточняющие вопросы — родными механизмами, без обходных путей. |
| **Всё на русском** | Документация и этот README — по-русски, без необходимости переводить. |

**Две доработки под автономность** — доктрина живости (поведенческая дисциплина
против зависших помощников; пока защищает прежде всего от потери работы при
компакции, полноценный рантайм-сторож — в планах) и фоновый веер-диспатч (свободный
ввод, пока субагенты работают параллельно). В оригинале этого нет.

**Почему в целом лучше:** меньше «вечного ожидания» и потерянной работы на длинных
задачах, результат предсказуемее — и по-прежнему без внешних зависимостей, только
встроенные средства.
...
| **dispatching-parallel-agents** | 2+ независимых задач | веер субагентов / Workflow, без общего состояния; **фоновый диспатч с yield+harvest** (см. `dispatch-yield-doctrine.md`) — ход возвращается человеку, пока веер работает |
| **using-git-worktrees** | только по явной просьбе про worktree | изолированная рабочая копия вместо ветки; иначе — `using-git-branches` |
...
| **frontend-design** | строишь / переделываешь UI, чтобы не выглядел шаблонно | эстетическое направление, типографика, анти-слоп (model-invoked) |
| **web-interface-guidelines** | аудит готового UI | прогон по прод-чек-листу (доступность, формы, состояния), находки как `file:line` (model-invoked) |
| **webapp-testing** | тестируешь / проверяешь локальное веб-приложение | прогон через Playwright: навигация, скриншоты, console-логи, проверка поведения UI (model-invoked) |
...
(автоматическое vs ручное ревью),
[`liveness-doctrine.md`](plugins/superpowers-claude/docs/liveness-doctrine.md)
(обнаружение зависших фоновых субагентов — поведенческие триггеры; рантайм-мониторинг
пока не реализован) и
[`dispatch-yield-doctrine.md`](plugins/superpowers-claude/docs/dispatch-yield-doctrine.md)
(фоновый веер → отдать ход → собрать результаты; свободный ввод при независимом
веере N≥2).
...
- **[сделано] Фоновый веер-диспатч** — независимый веер (N≥2) диспатчится в фон с
  yield+harvest (`dispatch-yield-doctrine.md`): ввод не блокируется, пока субагенты
  работают. Дальше — то же поведение завязать на реальный wall-clock floor (см. ниже).
- **[высокий] Реализовать живость в коде** — сейчас liveness это доктрина + текстовые
  триггеры; нужен реальный пассивный wall-clock floor по `inflight[]`, тикающий вне
  зависимости от того, молчит ли человек, чтобы зависший субагент детектился и без
  его участия.
